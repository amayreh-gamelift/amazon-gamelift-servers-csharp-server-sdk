...
#pragma warning restore S3242
        ...
        public GenericOutcome InitializeNetworking(ServerParameters serverParameters)
        {
            ...
            var computeType = Environment.GetEnvironmentVariable(EnvironmentVariableComputeType);
            ...
            var awsRegion = Environment.GetEnvironmentVariable(EnvironmentVariableAwsRegion) ?? serverParameters.AwsRegion;
            var accessKey = Environment.GetEnvironmentVariable(EnvironmentVariableAccessKey) ?? serverParameters.AccessKey;
            var secretKey = Environment.GetEnvironmentVariable(EnvironmentVariableSecretKey) ?? serverParameters.SecretKey;
            var sessionToken = Environment.GetEnvironmentVariable(EnvironmentVariableSessionToken) ?? serverParameters.SessionToken;
            var isContainerComputeType = ComputeTypeContainer.Equals(computeType);

            if (string.IsNullOrEmpty(websocketUrl))
            {
                return new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION, "websocketUrl is required"));
            }
            if (AgentlessContainerProcessId.Equals(Environment.GetEnvironmentVariable(EnvironmentVariableProcessId)))
            {
                processId = Guid.NewGuid().ToString();
            }
            if (string.IsNullOrEmpty(processId))
            {
                return new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION, "processId is required"));
            }
            if (!isContainerComputeType && string.IsNullOrEmpty(hostId))
            {
                return new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION, "hostId is required"));
            }
            if (string.IsNullOrEmpty(fleetId))
            {
                return new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION, "fleetId is required"));
            }

            var authTokenPassed = !string.IsNullOrEmpty(authToken);
            var sigV4ParametersPassed = !string.IsNullOrEmpty(awsRegion)
            ...
            if (!authTokenPassed && !sigV4ParametersPassed && !isContainerComputeType)
            {
                return new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION,
                    "Either AuthToken or AwsRegion and AwsCredentials are required"));
            }

            if (authTokenPassed)
            {
                return EstablishNetworking(websocketUrl, authToken, null);
            }
            else
            {
                ...
                var sigV4QueryString = GetSigV4QueryString(awsRegion, accessKey, secretKey, sessionToken);
                return EstablishNetworking(websocketUrl, authToken, sigV4QueryString);
            }
        }
        ...
        public void OnErrorResponse(string requestId, int statusCode, string errorMessage, string action)
        {
            switch (action)
            {
                ...
                {
                    OnActivateServerProcess(statusCode);
                    break;
                }
            }
        ...
        public void OnActivateServerProcess(int statusCode)
        {
            // Range for success status code is [200, 299]
            if (statusCode >= HttpStatusCodeSuccessStart && statusCode <= HttpStatusCodeSuccessEnd)
            {
            ...
            {
                Log.WarnFormat("Marking processReady as false. ActivateServerProcess() failed with HTTP status code {0}.", statusCode);
                processReady = false;
            }
        }
